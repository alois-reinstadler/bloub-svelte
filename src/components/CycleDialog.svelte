<script lang="ts">
  import { t } from '@/i18n'
  import { GIF_BACKGROUNDS, CYCLE_FORMATS, cycleSupportsTransparency, canEncodeVideo, type GifBackground, type CycleFormat } from '@/ui/export'
  let { open = $bindable(), format = $bindable(), fond = $bindable(), avancement, erreur, onconfirm, onannuler }: {
    open: boolean; format: CycleFormat; fond: GifBackground; avancement: number | null; erreur: boolean; onconfirm: () => void; onannuler: () => void
  } = $props()
  const formats = CYCLE_FORMATS.filter((item) => item !== 'mp4' || canEncodeVideo())
