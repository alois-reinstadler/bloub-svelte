<script lang="ts">
  ...
  let { etat, onexporter }: { etat: EtatExport; onexporter: (id: ActionId) => void } = $props()
  let open = $state(false)
  let root: HTMLElement
  const actions = ACTIONS.filter((action) => action.mode !== 'copieImage' || copiePossible())
  let busy = $derived(etat === 'occupe')
  let label = $derived(etat === 'exporte' ? t('export.done') : etat === 'copie' ? t('export.copied') : etat === 'erreur' ? t('export.failed') : t('export.action'))
  let confirmed = $derived(etat === 'exporte' || etat === 'copie')
    ...
    <div class="absolute right-0 bottom-full z-10 mb-2 w-60 rounded-xl border border-[var(--line)] bg-[var(--paper)] p-1 shadow-lg">
      {#each actions as action (action.id)}
        <button type="button" class="flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm transition hover:bg-black/5 {action.id === 'copie' ? 'mt-1 border-t border-[var(--line)] pt-2.5' : ''}" onclick={() => launch(action.id)}>
          <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true" class="shrink-0 text-[var(--muted)]"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">{#if action.mode === 'copieImage' || action.mode === 'copieTexte'}<path d="M6 11c0-2.828 0-4.243.879-5.121C7.757 5 9.172 5 12 5h3c2.828 0 4.243 0 5.121.879C21 6.757 21 8.172 21 11v5c0 2.828 0 4.243-.879 5.121C19.243 22 17.828 22 15 22h-3c-2.828 0-4.243 0-5.121-.879C6 20.243 6 18.828 6 16v-5Z"/><path d="M6 19a3 3 0 0 1-3-3v-6c0-3.771 0-5.657 1.172-6.828C5.343 2 7.229 2 11 2h4a3 3 0 0 1 3 3"/>{:else}<path d="M3 15c0 2.828 0 4.243.879 5.121C4.757 21 6.172 21 9 21h6c2.828 0 4.243 0 5.121-.879C21 19.243 21 17.828 21 15"/><path d="M12 3v13m-4-4.375L12 16l4-4.375"/>{/if}</g></svg>
          {t(`export.${action.id}`)}
        </button>
      {/each}
    </div>
