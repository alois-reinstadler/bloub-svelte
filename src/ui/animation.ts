...
/**
 * Animate every projected face feature in the renderer's standalone SVG.
 * The body retains its first frame; eyes and mouth share timing and origin.
 * Optional paths preserve curved mouth foreshortening alongside the matrices.
 * Matching path topology lets the browser interpolate between captured frames.
 */
export function animatedSvg(base: string, matrices: string[][], duree: number, paths?: string[][]): string {
  ...
  // Transform-bearing mask paths are the eyes and mouth, in document order.
  let n = 0
  ...
  if (matrices.some(frame => frame.length !== n) ||
      (paths && (paths.length !== matrices.length || paths.some(frame => frame.length !== n)))) {
    throw new Error('Inkonsistente Gesichtsanimation')
  }
  const selectors = Array.from({ length: n }, (_, index) => `.oeil${index}`).join(',')
  const pas = 100 / (matrices.length - 1)
  const regles = Array.from({ length: n }, (_, oeil) => {
    const etapes = matrices
      .map((m, i) => `${+(i * pas).toFixed(3)}%{transform:${m[oeil]}${paths ? `;d:path("${paths[i]![oeil]}")` : ''}}`)
      .join('')
  ...
  const style =
    ...
    `${selectors}{transform-box:view-box;transform-origin:0 0;` +
    ...
    regles.join('') +
    `@media (prefers-reduced-motion:reduce){${selectors}{animation-play-state:paused}}` +
    '</style>'
